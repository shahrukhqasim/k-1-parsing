...
void process(Mat&image,vector<ValueData>&valueData) {
  io::CSVReader<7> in("model.csv");
  in.read_header(io::ignore_extra_column, "Page", "Left", "Top","Width","Height","FieldName","MappedFieldName");
  int page;
  float left;
  float top;
  float width;
  ...
  while(in.read_row(page, left, top,width,height,fieldName,mappedFieldName)){
    //Scalar color = Scalar( rng.uniform(0, 255), rng.uniform(0,255), rng.uniform(0,255) );
    ...
    //rectangle( image, tll, brr, color, 2, 8, 0 );
  }
}
...
void match(vector<KeyData>keyData,vector<ValueData>valueData, Mat&image) {
  ...
  for(int i=0;i<valueData.size();i++) {
    ...
    for(int j=0;j<keyData.size();j++) {
      cout<<"Comparing "<<valueData[i].exactKeyText<<" with "<<keyData[j].text<<endl;
      int newDistance=editDistanceCalculator.lDistance(valueData[i].exactKeyText.c_str(),keyData[j].text.c_str());
    ...
    if(minIndex!=-1) {
      cout<<"Found "<<valueData[i].exactKeyText<<"for "<<keyData[minIndex].text<<endl;
      Scalar color = Scalar( rng.uniform(0, 255), rng.uniform(0,255), rng.uniform(0,255) );
      rectangle( image, keyData[minIndex].p1, keyData[minIndex].p2, color, 2, 8, 0 );
