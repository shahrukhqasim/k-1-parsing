...
void HelperMethods::conCompFast(cv::Mat &img, std::vector<cv::Rect> &rboxes, float max_x, float max_y, float min_area, int type) {
...
void HelperMethods::binarizeShafait(Mat &gray, Mat &binary, int w, double k){
    Mat sum, sumsq;
    gray.copyTo(binary);
    int half_width = w >> 1;
...
void HelperMethods::invertImage(Mat &image) {
