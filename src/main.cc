...
void printHelp() {
    ...
    cout<<" * You need to give path of a directory as an argument which must contain these folders:\n"
                  ...
                  " * 3. programOutput - empty for OCR+Accuracy test. And for accuracy test, should contain JSON files containing output of OCR. There should also be a ocrJsonFiles.txt file in the folder which specifies the JSON output files.\n"
                  " * 4. plottedDataComparison - an empty folder. Output is written to it. The files that are already there will be overwritten.\n";
    cout<<"All files.txt should contain consecutive file names. First document has to appear at the top and so on"<<endl;
}
