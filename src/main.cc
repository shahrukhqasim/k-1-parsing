...
//void match(vector<OcrResult> keyData, vector<ValueData> valueData, Mat&image);
//
...
//    vector<OcrResult> data = finder.getRecognizedData();
...
//    Preprocessor::conCompFast(image,boxes,0.5,0.5,1024,4);
//    Preprocessor::invertImage(image);
//
//    Mat image2;
...
//    image=image3;
//
//    Preprocessor::binarizeShafait(image,image3,100,0.5);
//
//    image=image3;
//
//    vector<Rect>boxes;
//    Preprocessor::conCompFast(image,boxes,1,1,0,8);
//
...
int main() {
    ...
    for(int i=0;i<=0;i++) {
        ...
        Program runner(fileName,inputFolder,outputFolder);
        runner.run();
    }
	return 0;
}
